...
function autoExpandTimeline() {
	...
	if (window.scrollY - lastScrollY >= 0) {
		jQuery('.timeline-work').each((i, elem) => {
			if (!elem.classList.contains('hide') && window.scrollY >= (elem.offsetTop - 150)) {
				showIndex = i;
			}
		});
	} else {
		if (currentIndex > 0 && window.scrollY < jQuery('#timeline').get(0).offsetTop - 50) {
			showIndex = currentIndex - 1;
		}
	}

	if (showIndex != null && showIndex != currentIndex) {
